...
/**
 * probeCore-side desktop [PassportBridge] for the CLI.
 *
 * Same native `uniffi.ooniprobe` binding as `composeApp`'s `DesktopPassportBridge`, but takes the
 * User-Agent string directly instead of reading `composeApp`'s flavor config (`OrganizationConfig` /
 * generated `SharedBuildConfig`), keeping probeCore free of the composeApp dependency.
 *
 * Constructing this class does not touch the native library; the library is only loaded when one of
 * the request methods is invoked (which throws when the bundled native lib is absent, e.g. under
 * `desktopTest`).
 */
internal class CliDesktopPassportBridge(
