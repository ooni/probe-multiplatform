...
class EnhancedUpdateManager(
    ...
    override fun initialize(appcastUrl: String, publicKey: String?) {
    ...
    fun getDiagnostics(): UpdateDiagnostics? {
        return diagnostics?.collectDiagnostics(baseManager)
    }
    ...
    fun exportDiagnosticsJson(): String? {
        return diagnostics?.exportDiagnosticsJson(baseManager)
    }
    ...
    fun isRecovering(): Boolean {
        return errorRecovery?.isRecovering() ?: false
    }
    
    /**
     * Get recovery statistics
     */
    fun getRecoveryStats(): Pair<Int, Int>? {
        return errorRecovery?.getRetryStats()
    }
