...
suspend fun waitForBootstrap(preferences: PreferenceRepository) {
...
suspend fun skipOnboarding(preferences: PreferenceRepository) {
    waitForBootstrap(preferences)
...
suspend fun disableRefreshArticles(preferences: PreferenceRepository) {
    preferences.setValueByKey(SettingsKey.LAST_ARTICLES_REFRESH, Clock.System.now().epochSeconds)
}

suspend fun defaultSettings(preferences: PreferenceRepository) {
    preferences.setValuesByKey(
        listOf(
