...
class ParseOonirunApiErrorTest {
    private val parseOonirunApiError = ParseOonirunApiError(Dependencies.buildJson())

    @Test
    fun parsesFieldViolationObjectDetail() {
        val error = parseOonirunApiError(422, """{"detail": {"error": "targetsName cannot be combined with inputs"}}""")
    ...
    fun parsesDefaultPydanticValidationList() {
        ...
        val error = parseOonirunApiError(422, body)
    ...
    @Test
    fun parsesForbidden() {
        val error = parseOonirunApiError(403, """{"detail": "OONI Run link has expired and cannot be edited"}""")
    ...
    fun parsesNotFoundRegardlessOfBody() {
        val error = parseOonirunApiError(404, """{"detail": "OONI Run link not found"}""")
    ...
    fun parsesRateLimitWithoutDetailKey() {
        val error = parseOonirunApiError(429, """{"error": "quota exceeded"}""")
    ...
    fun fallsBackToUnknownForUnrecognizedShape() {
        val error = parseOonirunApiError(500, "internal server error")
