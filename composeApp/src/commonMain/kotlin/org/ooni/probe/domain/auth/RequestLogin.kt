...
class RequestLogin(
    ...
    private val decodeAuthResponse: (
        PassportHttpResponse,
        DeserializationStrategy<OoniAuthLoginResponse>,
    ) -> Result<OoniAuthLoginResponse, AuthException>,
    ...
    suspend operator fun invoke(emailAddress: String): Result<OoniAuthLoginResponse, AuthException> {
        ...
        return passportPost("${OrganizationConfig.ooniApiBaseUrl}/api/v2/ooniauth/user-login", body)
            ...
            .flatMap { decodeAuthResponse(it, OoniAuthLoginResponse.serializer()) }
    }
}
