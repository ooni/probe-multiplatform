...
fun Navigation(
    ...
    NavHost(
        ...
        composable<Screen.Descriptors> {
            val viewModel = viewModel {
                dependencies.descriptorsViewModel(
                    ...
                    goToCreateDescriptor = { navController.safeNavigate(Screen.CreateDescriptor()) },
                )
            }
            val state by viewModel.state.collectAsState()
        ...
        composable<Screen.CreateDescriptor> { entry ->
            val loginToken = entry.toRoute<Screen.CreateDescriptor>().loginToken
            val viewModel = viewModel {
                dependencies.createDescriptorViewModel(
                    loginToken = loginToken,
                    onBack = { navController.goBack() },
                    onDescriptorCreated = {
                        navController.goBackAndNavigate(Screen.Descriptor(it.value))
                    },
                )
            }
            val state by viewModel.state.collectAsState()
            CreateDescriptorScreen(state, viewModel::onEvent)
        }
        ...
        composable<Screen.Descriptor> { entry ->
            val viewModel = viewModel {
                dependencies.descriptorViewModel(
                    ...
                    goToResult = { navController.safeNavigate(Screen.Result(it.value)) },
                    goToDescriptorWebsites = {
                        navController.safeNavigate(Screen.DescriptorWebsites(it.value))
                    },
                )
            }
            val state by viewModel.state.collectAsState()
        ...
        composable<Screen.DescriptorWebsites> { entry ->
            ...
            val state by viewModel.state.collectAsState()
            DescriptorWebsitesViewModel(state, viewModel::onEvent)
        }
