...
private fun DateFilter(
    ...
    FlowRow(
        ...
        ResultFilterChip(
            text = customDate
                ?.let {
                    stringResource(
                        Res.string.TestResults_Filter_Date_Custom_Filled,
                        it.customRange.start.isoFormat() + " – " + it.customRange.endInclusive.isoFormat()
                    )
                }
    ...
    if (showDatePicker) {
        ...
        val dateRangePickerState = rememberDateRangePickerState(
            initialSelectedStartDateMillis = initialRange?.start?.toEpoch(),
            initialSelectedEndDateMillis = initialRange?.endInclusive?.toEpoch(),
        )
        ...
        DatePickerDialog(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        val start =
                            dateRangePickerState.selectedStartDateMillis?.toLocalDateTime()?.date
                                ?: return@TextButton
                        val end =
                            dateRangePickerState.selectedEndDateMillis?.toLocalDateTime()?.date
                                ?: return@TextButton
                        updateFilter(
